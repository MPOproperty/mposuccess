	...
	<div class="col-md-12">
		<div class="portlet">
			<div class="portlet-title">
				<div class="caption">
					<i class="fa fa-shopping-cart"></i>@lang('mposuccess::payment.createPayment')
				</div>
				<div class="actions btn-set">
					<a href="/panel/admin/payments" class="btn default">
						<i class="fa fa-angle-left"></i> @lang('mposuccess::panel.back')
					</a>
				</div>
			</div>
				...
				<div class="tabbable">
					...
					<ul class="nav nav-tabs">
						<li @if (!in_array(Input::old('_tab'), [2,3])) class="active" @endif>
							...
							@lang('mposuccess::payment.intakeExternalPayment') </a>
						</li>
						<li @if (Input::old('_tab') == 3) class="active" @endif>
							<a href="#tab_balanse3" data-toggle="tab">
							@lang('mposuccess::payment.accountTransferPayment') </a>
						</li>
					</ul>
					<div class="tab-content no-space">
						{{--Tab 1--}}
						<div class="tab-pane @if (!in_array(Input::old('_tab'), [2,3])) active @endif>" id="tab_balanse">
							<form method="post" action="{{ route(config('mposuccess.admin_prefix') . '.payments.intakeExternalPayment') }}" enctype="multipart/form-data">
							...
							<div class="form-body">
								...
								<div class="form-group @if($errors->has('user_to')) has-error @endif">
									<label class="control-label">@lang('mposuccess::payment.paymentForm.user')</label>
									<select id="select2_program" class="form-control" name="user_to" data-placeholder="@lang('mposuccess::payment.selectUser')" >
								...
								<div class="form-group">
									<button type="submit" class="btn green-haze">
										@lang('mposuccess::payment.saveChanges')
									</button>
								</div>
							</div>
							</form>
						...
						<div class="tab-pane @if (Input::old('_tab') == 3) active @endif>" id="tab_balanse3">
							<form method="post" action="{{ route(config('mposuccess.admin_prefix') . '.payments.accountTransferPayment') }}" enctype="multipart/form-data">
							...
							<div class="form-body">
								...
								<div class="form-group @if($errors->has('user_transfer_from')) has-error @endif">
									<label class="control-label">@lang('mposuccess::payment.paymentForm.user_from')</label>
									<select id="select2_program" class="form-control" name="user_transfer_from" data-placeholder="@lang('mposuccess::payment.selectUser')" >
										 @foreach ($users as $key => $user)
								...
								<div class="form-group @if($errors->has('user_transfer_to')) has-error @endif">
									...
									<select id="select2_program" class="form-control" name="user_transfer_to" data-placeholder="@lang('mposuccess::payment.selectUser')" >
										 @foreach ($users as $key => $user)
