        ...
        <div class="col-md-3 col-sm-4 col-xs-6">
            <div class="product-item">
                <div class="pi-img-wrapper">
                    <img src="<?=count($array_images) ? config('mpouspehm.entities_storage_img') . $array_images[0] : ''?>" class="img-responsive" alt="">
                    <div>
                        @if (count($array_images))
                        <a href="{{config('mpouspehm.entities_storage_img') . $array_images[0]}}" class="btn btn-default fancybox-button">@lang('mpouspehm::panel.zoom')</a>
                        @endif
                        <a href="#product-pop-up" class="btn btn-default fancybox-fast-view" data-images="{{implode(",", $array_images)}}"
                           data-count="{{$product->count}}" data-desc="{{$product->des . '<br>' . $description_entities}}">@lang('mpouspehm::panel.more')</a>
                    </div>
                </div>
                <h3>{{$product->name}}</h3>
                <div class="pi-price"><span><i class='fa fa-rouble'></i></span>{{$product->coast}}</div>
                <a href="/bye/tree/{{$product->url}}" class="btn btn-default add2cart not-active">@lang('mpouspehm::panel.buy')</a>
                <div class="sticker sticker-level"><span>@lang('mpouspehm::profile.structures.'.$product->level )</span></div>
            </div>
        </div>
    @endforeach
</div>
        ...
        <div class="row">
            ...
            <div class="col-md-6 col-sm-6 col-xs-9">
                ...
                <div class="product-page-cart">
                    ...
                    <a href="javascript:;" class="btn btn-default add2cart not-active">@lang('mpouspehm::panel.buy')</a>
...
<script>
    var storage_img_entities = "{{config('mpouspehm.entities_storage_img')}}";
</script>