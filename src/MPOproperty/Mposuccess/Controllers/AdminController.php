...
use MPOproperty\Mposuccess\Requests\AccountTransferRequest;
use MPOproperty\Mposuccess\Requests\DeductionExternalRequest;
use MPOproperty\Mposuccess\Requests\IntakeExternalRequest;
...
use MPOproperty\Mposuccess\Models\Transaction;
...
use MPOproperty\Mposuccess\Repositories\Catalog\EntityRepository;
use MPOproperty\Mposuccess\Repositories\Role\RoleRepository;
...
use MPOproperty\Mposuccess\Repositories\Tree\TreeSettingsRepository;
use Auth;
...
class AdminController extends ProfileController {
    ...
    public function updateNews(Request $request, NewsRepository $newsRepository, $id = null)
    {
        if ($request->isMethod('post')) {
            ...
            if ($id) { /* edit */
                $headerTitle = trans('mposuccess::panel.news.edit');
                $news = $newsRepository->find($id);
            } else { /* add */
                $headerTitle = trans('mposuccess::panel.news.add');
                $news = null;
            }

            return view('mposuccess::admin.newsUpdate', [
                'news' => $news,
        ...
        StatusTransactionRepository $statusTransactionRepository,  Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            $this->layout->title = trans('mposuccess::payment.transactions');
            return $this->layout;
    ...
    public function createPayments(UserRepository $user)
    {
        ...
        $this->layout->content = view("mposuccess::admin.payment.create", $data);

        $this->layout->title = trans('mposuccess::admin.payments.create');
    ...
    )
    {
        ...
        DBTransaction::transaction(function() use ($data, $status, $payment, $type, $transaction) {
            ...
            $transactionData = [
                ...
                'user' => $this->user->id,
                'description' => trans('mposuccess::payment.descriptions.INTAKE_EXTERNAL'),
            ...
            $payment->setPaymentNotification(
                $data['user_to'],
                trans('mposuccess::payment.notificationNames.INTAKE_EXTERNAL'),
                trans('mposuccess::payment.notificationMessages.INTAKE_EXTERNAL', array('sum' => $data['price_intake']))
            );
        });

        RequestDefault::session()->reflash();
        RequestDefault::session()->flash('alert-success', trans('mposuccess::payment.success-flash-operation'));
    ...
    )
    {
        ...
        DBTransaction::transaction(function() use ($data, $status, $payment, $type, $transaction) {
            ...
            $transactionData = [
                ...
                'user' => $this->user->id,
                'description' => trans('mposuccess::payment.descriptions.DEDUCTION_EXTERNAL'),
            ...
            $payment->setPaymentNotification(
                $data['user_from'],
                trans('mposuccess::payment.notificationNames.DEDUCTION_EXTERNAL'),
                trans('mposuccess::payment.notificationMessages.DEDUCTION_EXTERNAL', array('sum' => $data['price_deduction']))
            );
        });
    ...
    )
    {
        ...
        DBTransaction::transaction(function() use ($data, $status, $payment, $type, $transaction) {
            ...
            $transactionData = [
                ...
                'user' => $this->user->id,
                'description' => trans('mposuccess::payment.descriptions.ACCOUNT_TRANSFER'),
    ...
    {
        $this->layout->content = view("mposuccess::admin.helperSettings");
        $this->layout->title = trans('mposuccess::admin.helperSettings');
        return $this->layout;
    }
    /**
    ...
    public function userAction(Request $request, RoleRepository $roleRepository, ProgramRepository $programRepository, CountryRepository $countryRepository, UserRepository $userRepository, $id, $action)
    {
        switch($action) {
            case 'view':
                return view("mposuccess::admin.user.view", [
                    'user' => $userRepository->find($id)
                ]);
            case 'edit':
                return view("mposuccess::admin.user.edit", [
                    'user'      => $userRepository->find($id),
    ...
    public function withdrawal(
        ...
        if ($request->isMethod('get')) {
            ...
            $this->layout->content = view("mposuccess::admin.withdrawal", [
                'statuses' => $statusTransactionRepository->getWithdrawalStatus(),
                'methods' => $types->lists('name', 'id'),
            ]);

            $this->layout->title = trans('mposuccess::admin.withdrawal');
        ...
        if($withdrawalModel = $withdrawal->changeWithdrawalStatus($id, $status::STATUS_PAID)){
            ...
            DBTransaction::transaction(function() use ($withdrawalModel, $status, $transaction, $type, $payment) {
                ...
                $transactionData = [
                    ...
                    'description' => trans('mposuccess::payment.notificationNames.WITHDRAWAL'),
                ...
                $payment->setPaymentNotification(
                    ...
                    trans('mposuccess::payment.notificationMessages.WITHDRAWAL_PAYMENT', array('sum' => $withdrawalModel->price))
                );
            });
    ...
    public function withdrawalStatusRejected(WithdrawalRepository $withdrawal, PaymentRepository $payment, $id)
    {
        ...
        if($price = $withdrawal->changeWithdrawalStatus($id, $status::STATUS_REJECTED)) {
            $payment->setPaymentNotification(
                $this->user->id,
                trans('mposuccess::payment.notificationNames.WITHDRAWAL'),
                trans('mposuccess::payment.notificationMessages.WITHDRAWAL_REJECTED', array('sum' => $price))
            );
    ...
    public function entities(Request $request, EntityRepository $entityRepository)
    {
        if ($request->isMethod('get')) {
            ...
            $this->layout->content = view("mposuccess::admin.entities");
            $this->layout->title = trans('mposuccess::admin.entities');
            return $this->layout;
    ...
    public function entityAction(Request $request, EntityRepository $entityRepository, $id, $action)
    {
        switch($action) {
            case 'add':
                return view("mposuccess::admin.entity.add");
            case 'edit':
                return view("mposuccess::admin.entity.edit", [
    ...
    public function updateProduct(Request $request, ProductRepository $productRepository, EntityRepository $entityRepository, $id = null)
    {
        if ($request->isMethod('post')) {
            ...
            if ($id) { /* edit */
                $headerTitle = trans('mposuccess::panel.products.edit');
                $product = $productRepository->find($id);
            } else { /* add */
                $headerTitle = trans('mposuccess::panel.products.add');
                $product = null;
            }

            return view('mposuccess::admin.productUpdate', [
                'product'       => $product,
                'headerTitle'   => $headerTitle,
