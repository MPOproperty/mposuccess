...
class TransactionRepository extends Repository
{
    ...
    function model()
    {
        return 'MPOproperty\Mposuccess\Models\Transaction';
    }
        ...
        if(!$isAllField) {
            ...
            foreach ($result['transactions'] as $transaction) {
                $records["data"][] = array(
                    ...
                    date_format(date_create($transaction->created_at), 'd M Y H:i:s'),
                    ''
//                    '<a href="/panel/admin/payments/' . $transaction->id . '/edit" class="btn default btn-xs purple b-edit" data-target="#ajax" data-toggle="modal"><i class="fa fa-edit"></i>' . trans('mposuccess::panel.edit') . '</a>'
                );
            }
        }
    ...
    public function transactionRefillBalance($userId, $price, $description = null)
    {
        DBTransaction::transaction(function () use ($userId, $description, $price) {
            ...
            $transactionData = [
                ...
                'user' => $userId,
                'description' => isset($description) ? $description : trans('mposuccess::payment.descriptions.INTAKE_INTERNAL'),
                'price' => $price,
                'from' => 0,
                'to' => 0
            ];
    ...
    public function transactionWithdrawal($userId, $price)
    {
        DBTransaction::transaction(function () use ($userId, $price) {
            ...
            $transactionData = [
                ...
                'user' => $userId,
                'description' => trans('mposuccess::payment.descriptions.TYPE_WITHDRAWAL'),
                'price' => $price,
                'from' => 0,
                'to' => 0
            ];
