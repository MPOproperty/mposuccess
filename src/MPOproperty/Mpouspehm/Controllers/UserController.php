...
use MPOproperty\Mpouspehm\Repositories\Payment\ConclusionRepository;
...
use MPOproperty\Mpouspehm\Repositories\User\Criteria\Current;
...
class UserController extends Controller {
     ...
     * @var string
     */
    protected $layout = "mpouspehm::layouts.panel.main";
    ...
    public function __construct(Request $request,
        ...
        if ($this->user->refer == '') {
            $this->user->refer = config('mpouspehm.company_id');
        }
        ...
        if ( ! is_null($this->layout))
        {
            ...
            if($this->user->is('bad.user')){
                $this->layout->slidebar = view('mpouspehm::profile.layout.user.slidebar');
    ...
    public function personal(CountryRepository $country, ProgramRepository $program, UserRepository $userRepository)
    {
        ...
        TreeRepository::setTable('MPOproperty\Mpouspehm\Models\Tree1');
        $treeRepository = app('MPOproperty\Mpouspehm\Repositories\Tree\TreeRepository');
        ...
        $this->layout->content = view("mpouspehm::profile.personal", $data);
        $this->layout->title = trans('mpouspehm::profile.myProfile');
        return $this->layout;
    }
    ...
    public function user($id, CountryRepository $countryRepository, ProgramRepository $programRepository, UserRepository $userRepository)
    {
        ...
        if ($user->refer == '') {
            $user->refer = config('mpouspehm.company_id');
        }
        ...
        if ($this->id != 1) {
            $data['refer'] =  app('MPOproperty\Mpouspehm\Repositories\User\UserRepository')->getRefer();
        }

        \Assets::add('profile-other');

        return view("mpouspehm::profile.user", $data);

        /*$this->layout->content =
        $this->layout->title = trans('mpouspehm::profile.user') . ' ' . $user->name;
        return $this->layout;*/
    }
    ...
    public function dashboard(UserRepository $userRepository)
    {
        ...
        $this->layout->content = view("mpouspehm::profile.dashboard", $data);
        $this->layout->title = trans('mpouspehm::profile.personal');
        return $this->layout;
    }
    ...
    public function newsPrivate(NewsRepository $newsRepository)
    {
        ...
        $news = $newsRepository->findByTypeAndPaginate(config('mpouspehm.news_type_private'), $perPage);

        $this->setNewsBackLink('news-panel-link-back');

        $this->layout->content = view("mpouspehm::profile.news", [
            'news' => $news
        ]);
        $this->layout->title = trans('mpouspehm::profile.news');
        return $this->layout;
    }
    ...
    {
        $news = $newsRepository->findByIdAndType($id, config('mpouspehm.news_type_private'));
    ...
    {
        $this->layout->content = view("mpouspehm::profile.score.refill");
        $this->layout->title = trans('mpouspehm::profile.score.refill');
        return $this->layout;
    }*/
    ...
    public function catalog(ProductRepository $products)
    {
        ...
        $this->layout->content = view("mpouspehm::profile.catalog", [
        ...
        $this->layout->title = trans('mpouspehm::profile.catalog');
        return $this->layout;
    }
    ...
    public function partner(CountryRepository $country, ProgramRepository $program, UserRepository $userRepository)
    {
        ...
        $this->layout->content = view("mpouspehm::profile.partner", $data);
        $this->layout->title = trans('mpouspehm::profile.partnerProgram.title');
        return $this->layout;
    }
    ...
    public function operations()
    {
        ...
        $this->layout->content = view("mpouspehm::profile.score.operations", $data);
        $this->layout->title = trans('mpouspehm::profile.score.operations');
    ...
    public function operationTransfer(UserRepository $user)
    {
        $data = [
            'users' => $user->lists('sid', 'id'),
            'descriptionTransfer' => trans('mpouspehm::payment.descriptionTransfer'),
        ];

        return view("mpouspehm::profile.score.transfer", $data);
    }
    ...
    public function operationСonclusion(ConclusionTypeRepository $types )
    {
        ...
        return view("mpouspehm::profile.score.conclusion", $data);
    }
    ...
    public function operationRefill(UserRepository $user, PaymentRepository $payment)
    {
        $data = [
            'users' => $user->lists('sid', 'id'),
            'descriptionRefill' => trans('mpouspehm::payment.descriptionRefillBalance'),
            'descriptionTax' => trans('mpouspehm::payment.descriptionTax'),
            'tax' => PaymentRepository::TAX
        ];

        return view("mpouspehm::profile.score.refill", $data);
    }
    ...
    )
    {
        if ($request->isMethod('get')) {
            ...
            $this->layout->content = view("mpouspehm::profile.score.purchases");
            $this->layout->title = trans('mpouspehm::profile.score.purchases');
    ...
    )
    {
        if ($request->isMethod('get')) {
            ...
            $this->layout->content = view("mpouspehm::profile.score.bonus");
            $this->layout->title = trans('mpouspehm::profile.score.bonuses');
    ...
    )
    {
        ...
        if ($request->isMethod('get')) {
            ...
            $this->layout->title = trans('mpouspehm::profile.score.myOperations');
    ...
    {
        $this->layout->content = view("mpouspehm::profile.score.places");
        $this->layout->title = trans('mpouspehm::profile.score.places');
        return $this->layout;
    }
    ...
    )
    {
        ...
        DBTransaction::transaction(function() use ($data, $status, $payment, $type, $transaction) {
            ...
            $transactionData = [
                ...
                'user' => $this->user->id,
                'description' => $data['description'],//trans('mpouspehm::payment.descriptions.ACCOUNT_TRANSFER'),
        ...
        return redirect(config('mpouspehm.panel_url') . '/score/operations')->with([
            'status' => 'success',
            'message' => trans('mpouspehm::payment.transferSuccess'),
            'name' => trans('mpouspehm::payment.notificationNames.ACCOUNT_TRANSFER'),
        ]);
    }
    ...
    public function refillBalance()
    {
        ...
        $transaction = app('MPOproperty\Mpouspehm\Repositories\Transaction\TransactionRepository');
        ...
        $this->layout->content = view("mpouspehm::profile.payment", [
            ...
            'text' => trans('mpouspehm::payment.linkText'),
            'sumDescription' => trans('mpouspehm::payment.refillDescriptionTax', array('sum' => $priceWithTax)),
        ]);
    ...
    )
    {
        ...
        $payment->setPaymentNotification(
            ...
            trans('mpouspehm::payment.notificationMessages.WITHDRAWAL', array('sum' => $data['price_conclusion']))
        );

        return redirect(config('mpouspehm.panel_url') . '/score/operations');
    }
}
