<?php namespace MPOproperty\Mpouspehm\Repositories\News;
/**
 * Created by PhpStorm.
 * User: nick
 * Date: 15.09.2015
...
class NewsRepository extends Repository {
    ...
    function model()
    {
        ...
        return 'MPOproperty\Mpouspehm\Models\News';
    }
    ...
    */
    ...
    public function findNumberLastPublic($number)
    {
        return $this->model
            ...
            ->whereRaw('type = ? or type = ?', [
                config('mpouspehm.news_type_company'),
                config('mpouspehm.news_type_world') ])
            ->orderBy('updated_at', 'desc')
            ->orderBy('id', 'desc')
            ->take($number)->get();
    }
    ...
    public function updateData($data, $request)
    {
        ...
        */
        ...
        {
            $destinationPath = config('mpouspehm.news_storage_img');
