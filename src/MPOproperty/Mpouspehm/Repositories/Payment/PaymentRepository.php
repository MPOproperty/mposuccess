...
use MPOproperty\Mpouspehm\Models\Payment;
...
class PaymentRepository extends Repository
{
    ...
    function model()
    {
        return 'MPOproperty\Mpouspehm\Models\Payment';
    }
    ...
    )
    {
        ...
        DBTransaction::transaction(function() use ($levelPrice, $userId, $status, $transaction, $type, $description) {
            ...
            $this->setPaymentNotification(
                $userId,
                trans('mpouspehm::payment.notificationNames.DEDUCTION_INTERNAL'),
                trans('mpouspehm::payment.notificationMessages.DEDUCTION_INTERNAL', array('sum' => $levelPrice))
            );
        });
    ...
    public function intakeExternalPayment($sum, $toUserId, $phaseDescription)
    {
        DBTransaction::transaction(function() use ($sum, $toUserId, $phaseDescription) {
            ...
            $this->setPaymentNotification(
                $toUserId,
                $phaseDescription,
                trans('mpouspehm::payment.notificationMessages.EXPULSION_COMPANY', array('sum' => $sum))
            );
        });
