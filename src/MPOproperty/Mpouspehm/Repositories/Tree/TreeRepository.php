...
use MPOproperty\Mpouspehm\Models\Tree;
...
class TreeRepository extends Repository{
    ...
    public function findIMindByUser($user, $parent, $level){
        ...
        if(0 == $parent) {
            ...
            $users = app('MPOproperty\Mpouspehm\Repositories\User\UserRepository');
    ...
    public function getCountReferralPlaces($id)
    {
        ...
        for ($level = 0; $level < config('mpouspehm.structure_count'); $level++) {
            self::$table = 'MPOproperty\Mpouspehm\Models\Tree' . ($level+1);
            $this->makeModel();
    ...
    public function getCountReferralPlacesInLevel($refId, $level) {
        self::$table = 'MPOproperty\Mpouspehm\Models\Tree' . ($level);
        $this->makeModel();
        return $this->model
            ->whereIn('user_id', \DB::table('users')
                ->where('refer', '=', $refId)
    ...
    {
        //return 'MPOproperty\Mpouspehm\Models\Tree';
        return self::$table;
    }
