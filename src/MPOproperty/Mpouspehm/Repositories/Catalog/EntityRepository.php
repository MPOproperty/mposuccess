...
use MPOproperty\Mpouspehm\Models\Entity;
...
class EntityRepository extends Repository {
    ...
    function model()
    {
        return 'MPOproperty\Mpouspehm\Models\Entity';
    }
    ...
    public function findByParamsForTable($params){
        ...
        foreach($result['entities'] as $entity) {
            $records["data"][] = array(
                ...
                ($entity->image != '') ? '<div class="thumbnail"><span data-id="' . $entity->id .'" class="glyphicon glyphicon-remove"></span><img src="' . config('mpouspehm.entities_storage_img') . $entity->image . '"/></div>' : '',
                $entity->name,
                $entity->description,
                '<div class="margin-bottom-5"><a href="/panel/admin/entity/' . $entity->id .'/edit" class="btn default btn-xs purple b-load" data-target="#ajax" data-toggle="modal"><i class="fa fa-edit"></i>' .  trans('mpouspehm::panel.edit') . '</a></div>' .
                '<a data-href="/panel/admin/entity/' . $entity->id .'/delete" class="btn default btn-xs black b-delete" data-id-news="{{$item->id}}"><i class="fa fa-trash-o"></i>' . trans('mpouspehm::panel.delete') . '</a>',
            );
        }
    ...
    public function updateData($data, $request, $id)
    {
        ...
        */
        ...
        {
            $destinationPath = config('mpouspehm.entities_storage_img');
