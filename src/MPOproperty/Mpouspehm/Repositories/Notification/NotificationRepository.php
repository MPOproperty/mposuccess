<?php namespace MPOproperty\Mpouspehm\Repositories\Notification;
/**
 * Created by PhpStorm.
 * User: nick
 * Date: 10.09.2015
...
class NotificationRepository extends Repository {
    ...
    function model()
    {
        ...
        return 'MPOproperty\Mpouspehm\Models\Notification';
    }
    ...
    public function findByParamsForTable($params){
        ...
        foreach($notifications as $notification) {
            $records["data"][] = array(
                ...
                $notification->status ?
                    '<span class="label label-sm label-success">' . trans('mpouspehm::panel.notification.status-read') . '</span>' :
                    '<span class="label label-sm label-warning">' . trans('mpouspehm::panel.notification.status-unread') . '</span>',
                '<a data-href="/panel/notification/' . $notification->id .'/delete" class="btn default btn-xs black b-delete"><i class="fa fa-trash-o"></i>' . trans('mpouspehm::panel.delete') . '</a>',
            );
        }
