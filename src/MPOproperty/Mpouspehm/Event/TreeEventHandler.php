 ...
 * Time: 20:42
 */
namespace MPOproperty\Mpouspehm\Event;

use Bican\Roles\Models\Role;
use Illuminate\Queue\InteractsWithQueue;
use Illuminate\Contracts\Queue\ShouldQueue;
use MPOproperty\Mpouspehm\BinaryTree\Sheet;
use MPOproperty\Mpouspehm\Models\Notification;
...
class TreeEventHandler {//implements ShouldQueue {
    ...
    public function __construct(){
        ...
        $this->payment = app('MPOproperty\Mpouspehm\Repositories\Payment\PaymentRepository');
    }
    ...
    protected function init($level) {
        $settings = app('MPOproperty\Mpouspehm\Repositories\Tree\TreeSettingsRepository');
    ...
    private function onTreeBye($name, $sid, $uid, $pid, $current, $next){
        ...
        if($user->is('bad.user')) {
            ...
            $settings = app('MPOproperty\Mpouspehm\Repositories\Tree\TreeUserSettings');
        ...
        TreeRepository::setTable('MPOproperty\Mpouspehm\Models\Tree' . $current);
    ...
    {
        $events->listen('tree.one.bye', '\MPOproperty\Mpouspehm\Event\TreeEventHandler@onTreeOneBye');
        $events->listen('tree.two.bye', '\MPOproperty\Mpouspehm\Event\TreeEventHandler@onTreeTwoBye');
        $events->listen('tree.three.bye', '\MPOproperty\Mpouspehm\Event\TreeEventHandler@onTreeThreeBye');

        $events->listen('tree.four.bye', '\MPOproperty\Mpouspehm\Event\TreeEventHandler@onTreeFourBye');
        $events->listen('tree.five.bye', '\MPOproperty\Mpouspehm\Event\TreeEventHandler@onTreeFiveBye');
        $events->listen('tree.six.bye', '\MPOproperty\Mpouspehm\Event\TreeEventHandler@onTreeSixBye');
    }
